...
def match_from_paths(conf: Dict,
    ...
    for idx, data in enumerate(tqdm(loader, smoothing=.1)):
        ...
        # if idx == 0:
            ...
            # print("##### data['image_size0']  ", data['image_size0'])
            # print("##### data['image_size1']  ", data['image_size1'])
            # print("##### data['image0']  ", data['image0'].shape)
            # print(data['image0'])
            # print("##### data['image1']  ", data['image1'].shape)
            # print(data['image1'])
            # print("##### data['keypoints0']  ", data['keypoints0'].shape)
            # print(data['keypoints0'])
            # print("##### data['keypoints1']  ", data['keypoints1'].shape)
            # print(data['keypoints1'])
            # print("##### data['scores0']  ", data['scores0'].shape)
            # print(data['scores0'])
            # print("##### data['scores1']  ", data['scores1'].shape)
            # print(data['scores1'])
            # print("##### data['descriptors0']  ", data['descriptors0'].shape)
            # print(data['descriptors0'])
            # print("##### data['descriptors1']  ", data['descriptors1'].shape)
            # print(data['descriptors1'])

            # data = {k: v for k, v in data.items() if not (k.startswith('score') or k.startswith('desc'))}
            # data = {k: v if k.startswith('image')
            #         else v[0].cpu().numpy() for k, v in data.items()}
            # print("##### data['keypoints0']  ", data['keypoints0'].shape)
            # print(data['keypoints0'])
            # print("##### data['keypoints1']  ", data['keypoints1'].shape)
            # print(data['keypoints1'])
            ...
            # print("##### pred['matches0']  ", pred['matches0'].shape)
            # print(pred['matches0'])
            # print("##### pred['matches1']  ", pred['matches1'].shape)
            # print(pred['matches1'])
            # print("##### pred['matching_scores0']  ", pred['matching_scores0'].shape)
            # print(pred['matching_scores0'])
            # print("##### pred['matching_scores1']  ", pred['matching_scores1'].shape)
            # print(pred['matching_scores1'])
    writer_queue.join()
